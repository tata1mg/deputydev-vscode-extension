...
export class BaseHttpEndpoint {
    constructor(public httpHost: string, public endpoint: string, public method: string, public defaultExtraHeaders: Record<string, string>) { }
}
...
export class BaseWebsocketEndpoint {
    ...
    constructor(public wsHost: string, public endpoint: string, public defaultExtraHeaders: Record<string, string>, messageHandlers: Array<(data: any) => Promise<void>> = []) {
...
export class BaseClient {
    ...
    constructor(httpHost?: string, wsHost?: string, defaultWebsocketMessageHandlers: Array<(data: any) => Promise<void>> = []) {
    ...
    createWebsocketEndpoint(endpoint: string, defaultExtraHeaders: Record<string, string> = {}, messageHandlers: Array<(data: any) => Promise<void>> = []): BaseWebsocketEndpoint {
        if (!this.wsHost) {
            throw new Error("WebSocket host is not defined");
        }
        return new BaseWebsocketEndpoint(this.wsHost, endpoint, defaultExtraHeaders, [...messageHandlers, ...this.defaultWebsocketMessageHandlers]);
    }
}
