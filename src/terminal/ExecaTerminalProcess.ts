...
import type { DDTerminal } from './types';
import { BaseTerminalProcess } from './BaseTerminalProcess';

export class ExecaTerminalProcess extends BaseTerminalProcess {
  private terminalRef: WeakRef<DDTerminal>;
  private aborted = false;
  private pid?: number;

  constructor(terminal: DDTerminal) {
    super();
  ...
  public get terminal(): DDTerminal {
    ...
    if (this.pid) {
      psTree(this.pid, async (err, children) => {
        if (!err) {
          ...
          console.error(`[ExecaTerminalProcess] Failed to get process tree for PID ${this.pid}: ${err.message}`);
        }
      });
