...
export class WorkspaceFileWatcher {
  ...
  private loadIgnorePatterns(): void {
    ...
    const additionalIgnore: string[] = config?.["VSCODE_FILE_IGNORE"]?.["EXLUDE_PATTERN"] || [];
    patterns = patterns.concat(additionalIgnore);

    // Exclude directories from configuration.
    const excludeDirs: string[] = config?.["VSCODE_FILE_IGNORE"]?.["EXCLUDE_DIRS"] || [];
    ...
    const excludeExts: string[] = config?.["VSCODE_FILE_IGNORE"]?.["EXCLUDE_EXTS"]  || [];
