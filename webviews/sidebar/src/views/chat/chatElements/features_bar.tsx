...
export default function FeaturesBar() {
  ...
  return (
    <div className='flex justify-center pl-3 pr-3'>
      <div className='flex flex-col border-t-2 border-r-2 border-l-2 border-gray-700 w-full rounded-t-md'>
        {showAllMCPServers && (
        ...
        {showMCPServerTools && showAllMCPServers && (
          <div className='bg-transparent h-full max-h-[150px] overflow-y-auto p-2 transition-all duration-300 ease-out'
            style={{
        ...
        <div className='flex justify-between m-1.5'>
          <div className='flex gap-2 items-center'>
            {(showAllMCPServers || showMCPServerTools) && (
              <button>
                <ArrowLeft className='h-4 w-4 hover:cursor-pointer hover:bg-slate-700 hover:bg-opacity-5'
                  onClick={() => handleBack()}
                />
              </button>
            )}
            <button className="flex gap-2 items-center" onClick={() => handleShowMCPServers()}>
          ...
          <div className='flex gap-2 items-center'>
            <RefreshCw className={`hover:bg-slate-400 hover:bg-opacity-10 hover:cursor-pointer h-4 w-4 ${refreshSpinning && "animate-spin"}`} onClick={() => handleRefreshMCPServers()} />
            <FilePenLine className='hover:cursor-pointer h-4 w-4 hover:bg-slate-700 hover:bg-opacity-5' onClick={() => openMCPConfig()} />
          </div>
        </div>
      </div>
    </div>
