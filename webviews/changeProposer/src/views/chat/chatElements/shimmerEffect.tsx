...
export function Shimmer() {
  ...
  return (
    <div role="status" className="mb-[10px] mt-[5px] flex items-center space-x-2">
      <span className="animate-pulse text-sm text-gray-500">{randomPhrase}</span>
      <div className="flex space-x-1">
        {Array.from({ length: 3 }, (_, index) => (
          <div
            key={index}
            className={`h-1 w-1 animate-pulse rounded-full bg-gray-500 ${index < activeDots ? '' : 'opacity-0'}`}
          ></div>
        ))}
      </div>
    </div>
